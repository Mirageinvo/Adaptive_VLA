...
успех при восьми переворотах 4 против 4. Утверждение «новый скрипт даёт те же
результаты» требует сверки ИСХОДОВ, а не средних, и раз оно попадает в текст,
то считается кодом с самопроверкой, а не разовой командой в терминале.

ЧТО ИМЕННО ПОДТВЕРЖДАЕТ НОЛЬ ПЕРЕВОРОТОВ: совпадение ЭПИЗОДИЧЕСКИХ ИСХОДОВ, и
только их. Побитового совпадения всей последовательности действий отсюда не
следует — две реализации могли разойтись внутри эпизода и всё равно привести к
одному исходу. Для замены одного набора ячеек другим этого достаточно,
называть это совпадением траектории нельзя.
...
def main():
    ...
    if tot_f == 0:
        print("\n    ИСХОДЫ совпали на каждой паре: второй набор даёт те же "
              "результаты, что первый (о совпадении траекторий это не "
              "говорит)")
    else:
        print("\n    исходы совпадают НЕ на всех парах — совпадение средних, "
              "если оно есть, равенством результатов не является")
    if a.require_identical and tot_f:
        # ОТКАЗ, А НЕ СТРОЧКА В ЛОГЕ. Утверждение «пересчёт даёт те же
        # исходы» либо проверяемо машиной, либо его нет.
        raise SystemExit(
