...
def main() -> None:
    ...
    res = {}
    t_start = time.time()
    # ОДНА КОНФИГУРАЦИЯ НА ПРОЦЕСС. Два независимых ограничения сходятся к
    # одному решению.
    # 1. Модель обязана грузиться ПОСЛЕ создания сред: fork после инициализации
    #    CUDA вешает процесс.
    # 2. Среды нельзя переиспользовать между конфигурациями: envs.reset
    #    восстанавливает начальное состояние НЕ ПОЛНОСТЬЮ, и результат зависит
    #    от порядка прогонов внутри процесса. Обнаружено так: ячейка
    #    ens=on,H=20 дала 10 успехов будучи восьмой в процессе и 9 будучи
    #    первой, при полностью детерминированной среде (два одинаковых
    #    запуска совпали посимвольно).
    # Вместе это означает: среды создаются один раз, модель после них, и
    # больше в процессе не запускается ничего. Официальный eval_libero.py
    # устроен так же — один task_id и один горизонт на вызов.
    if len(args.task_id) > 1 or len(args.horizons) > 1 or args.ensemble == "both":
        raise SystemExit(
            "одна задача, один горизонт и один режим усреднения на запуск.\n"
            "Причина: среды нельзя переиспользовать между конфигурациями "
            "(reset\nвосстанавливает состояние не полностью), а пересоздать "
            "их нельзя\nпосле загрузки модели (fork после CUDA вешает "
            "процесс).\nРазвёртку гоните циклом оболочки:\n"
            "  for H in 4 8 12 20; do for E in off on; do ... --horizons $H "
            "--ensemble $E; done; done")
    ...
    if args.n_envs > 1:
        print(f"\n  ВНИМАНИЕ ПРО БАТЧИРОВАНИЕ: сред {args.n_envs} в одном"
              " вызове.\n"
              "  Один вызов политики обслуживает все среды сразу, поэтому «мс\n"
              "  на действие» здесь АМОРТИЗИРОВАННАЯ величина, а не задержка\n"
              "  реакции одного робота. Отношения между горизонтами от этого\n"
              "  не страдают, но для заявлений о latency нужен --n-envs 1.")
    else:
        print("\n  Одна среда: «мс на действие» — настоящая однороботная\n"
              "  задержка, батчирования нет.")
