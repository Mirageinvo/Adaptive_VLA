...
БЕЗОПАСНЫЙ СТАРТ. Нулём инициализированы ТОЛЬКО веса проекций обратного
внедрения; вентили `alpha` стартуют с единицы. Обнулять и то, и другое нельзя —
тогда обе производные обращаются в ноль и ветвь не обучается никогда (см.
CodeFeedback). При нулевой проекции прямой проход всё равно тождественен, и при
выходах `(24,)` модель обязана выдать ТОЧНО те же токены и те же логиты, что
первый блок официальной BAR. Это проверяется, а не предполагается.
