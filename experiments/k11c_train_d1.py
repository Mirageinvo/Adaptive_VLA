...
def read_train(best, rows, probe_pos, oracle_pos, skipped=()):
    ...
    if p <= probe_pos:
        txt += (f".\n  ОБУЧЕННАЯ ГОЛОВА НЕ ПРЕВЗОШЛА ЛИНЕЙНЫЙ ЗОНД "
                f"({probe_pos:.1%}): три слоя с tanh не дали ничего сверх "
                f"линейной регрессии на тех же входах. Это не отменяет "
                f"наличие сигнала, но обесценивает выбор архитектуры головы")
    elif p < oracle_pos / 2:
