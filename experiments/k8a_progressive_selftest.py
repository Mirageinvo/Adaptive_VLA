...
Проверки без модели (`--selftest`, идут на CPU):
  ...
  2. Обратное внедрение при alpha=0 не меняет состояние, при alpha!=0 меняет,
     градиент доходит до проекции.
...
Проверки с моделью (`--ckpt`, нужен GPU):
  5. ТОЖДЕСТВО. При выходах (24,) и нулевых вентилях токены обязаны совпасть с
     первым блоком официальной BAR на 100%.
  ...
  8. Внедрение действительно меняет поток при ненулевом вентиле, и форма
     внедряемого тензора (B,16,768).
...
def selftest_cpu():
    ...
    print("самопроверка k8a (без модели) пройдена: LoRA стартует тождеством, "
          "внедрение при alpha=0 пусто и градиент до проекции доходит, "
          "straight-through даёт жёсткий код вперёд и мягкий градиент назад")
...
def main() -> None:
    ...
    print(f"\n  выходы {exits}:")
    ok = True
    for mode, want in (("fast", exits[0]), ("medium", exits[1]),
                       ("full", exits[2])):
        with torch.no_grad():
        ...
        print(f"    {mode:<7} слоёв эксперта {c['expert']:>3}, VLM {c['vlm']:>3}, "
              f"уровней {len(o['codes'])}  {'ок' if good else 'НЕ ТО'}")
    if not ok:
        raise SystemExit(
