...
ТОЧНОСТЬ ДЕКОДИРОВАНИЯ — здесь она решает исход. Модель выпущена в bfloat16, у
которого 8 бит мантиссы. Оценка порядка: при типичной величине действия 0.3
округление даёт 6e-4, ошибка на исполняемом окне 1.6e-2, и сдвиг MSE от одного
только округления имеет ст. откл. около 2% одиночного выигрыша. В Omega входят
четыре независимо округлённых слагаемых, то есть шум ~4% одиночного выигрыша,
тогда как порог из плана 1e-3 * g1 в СОРОК РАЗ меньше. Тест считал бы
округление нарушениями.

Поэтому декодирование идёт в float32 (кодек маленький, это дёшево), а порог
берётся как

    tau = max(1e-8, 1e-3 * median(g1), 3 * измеренный численный пол),

где пол измеряется сравнением float32 с float64 на одиночных наборах. Отдельно
печатается расхождение bf16 против float32 — оно показывает, насколько прежние
замеры были затронуты.

ВАЖНО ПРО ПРЕЖНИЕ ЗАМЕРЫ. Средние величины (Delta в K-4a2) от этого почти не
страдают: случайное округление усредняется по 1536 примерам и падает в ~39 раз.
Страдают именно ПОЭКЗЕМПЛЯРНЫЕ пороговые решения, то есть подсчёт нарушений.
...
def main() -> None:
    ...
    # Декодируем в float32: в bf16 округление сопоставимо с искомым эффектом.
    # Кодек маленький, копия дешёвая. Генерация кодов остаётся на bf16-модели —
    # там результат дискретный и округление ни на что не влияет.
    import copy
    ...
    with torch.no_grad():
        ...
        Ebf = projected_codebooks(tok, args.device)
        ...
        gbf = singles_at(tok, Ebf, torch.bfloat16)
        ...
        print(f"  bfloat16 против float64: макс. расхождение {dbf:.3e} "
              f"({dbf / max(gsc, 1e-30):.3%}) <- столько было бы без правки")
        print(f"  пол порога 3*float32 = {floor32:.3e}\n")
        del tok64, E64, Ebf, g64, g32, gbf
