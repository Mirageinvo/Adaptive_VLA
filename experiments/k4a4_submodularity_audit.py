...
def analyze(G, chg, epi, pos, task, floor_om, P, args, dh=None) -> None:
  ...
  субмодулярность» недоступна.""")
    ...
    # ВЫВОД ПРО СЛОИ ДОПУСТИМ ТОЛЬКО ПРИ ПРОЙДЕННОМ КОНТРОЛЕ. Разбиение по
    # выигрышу использует величины из числителя Omega = G(q) + G(r) - G(qr):
    # отбирая пары с малыми G(q), G(r), мы не ограничиваем G(qr), и Omega
    # смещается вниз механически. Независимое разбиение по ||dh|| эту связь
    # разрывает; если знаковая картина там не воспроизводится, слоевой вывод
    # делать нельзя, каким бы выразительным он ни выглядел.
    ctl = asym.get("hid"), asym.get("lod")
    if ctl[0] is None:
        print("""
  КОНТРОЛЬ НЕ ПРОВОДИЛСЯ (в NPZ нет ||dh||): слоевой вывод не делается.
  Разбиение по выигрышу использует величины из числителя Omega, поэтому без
  независимого разбиения отличить структуру от отбора нельзя.""")
    elif not (ctl[0] > 0.2 and ctl[1] < -0.2):
        print(f"""
  СЛОЕВОЙ ВЫВОД НЕ ПОДТВЕРЖДЁН. При разбиении по выигрышу асимметрия
  {asym['hi']:+.2f} / {asym['lo']:+.2f}, при независимом разбиении по ||dh|| —
  {ctl[0]:+.2f} / {ctl[1]:+.2f}. Знаковая картина не воспроизводится, значит
  она связана именно с ВЕЛИЧИНОЙ выигрыша, а не с важностью позиции, и
  объясняется отбором: при малых G(q), G(r) величина G(qr) ничем не
  ограничена, и Omega смещается вниз.
  Писать «сильные позиции систематически перекрываются» НЕЛЬЗЯ. Остаётся
  только то, что от слоёв не зависит: глобальная несубмодулярность и
  нарушения монотонности.""")
    elif asym["hi"] > 0.3 and asym["lo"] < -0.3:
        print("""
