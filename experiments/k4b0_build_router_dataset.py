...
того плотного прохода, который мы хотим сэкономить? Планка измерена на ЭТОМ
датасете (test, 16000 вмешательств, смешанный офсет по задачам): лучший
причинный baseline 0.291, оракулы — 0.701 (независимое ранжирование), 0.846
(жадный), 0.877 (точный). Прежние числа 0.40 / 0.79 / 0.91 / 0.93 относятся к
фазе A на 96 наблюдениях при едином офсете 3 и не переносятся.
...
def derive_labels(raw, split, P, kmax, tau_rel, gap_rel):
    ...
    Основная метрика — RMS, потому что ворота B1 (0.291 / 0.701 / 0.846 / 0.877)
