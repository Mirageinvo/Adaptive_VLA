...
def main() -> None:
    ...
    print("\n" + "=" * 78)
    # ОТНОШЕНИЕ план/держать — главная безразмерная величина. D(j) само по
    # себе нечитаемо: правильный ответ тоже уезжает со временем, и без этого
    # знаменателя плоская кривая выглядит как «устаревания нет», хотя на
    # деле план ловит всё большую долю движения. При больших j «держать»
    # практически равно уровню случайности.
    ratio = np.where(ch > 1e-12, c / np.maximum(ch, 1e-12), np.nan)
    # ОЧИЩЕННОЕ УСТАРЕВАНИЕ. D(1) — постоянное несогласие политики с собой на
    # соседних вызовах, к горизонту отношения не имеющее. Вычитается в
    # квадратуре как независимая составляющая.
    excess = np.sqrt(np.maximum(c ** 2 - c[1] ** 2, 0.0))
    out["ratio_curve"], out["excess_curve"] = ratio, excess
    hdr = (f"  {'j':>3}{'D_план':>10}{'D_держать':>11}{'план/держ':>11}"
           f"{'уловл':>7}{'сверх_пола':>12}{'D/|a|':>8}"
    ...
    for j in range(CHUNK):
        ...
        line = (f"  {j:>3}{c[j]:>10.4f}{ch[j]:>11.4f}{ratio[j]:>11.2f}"
                f"{(1 - ratio[j]):>6.0%}{excess[j]:>12.4f}{rel:>8.2f}"
    ...
    print(f"  план ловит долю сдвига: j=1 {1 - ratio[1]:.0%}, "
          f"j=8 {1 - ratio[8]:.0%}, j=19 {1 - ratio[19]:.0%}")
    print(f"  очищенное устаревание: j=4 {excess[4]:.3f}, j=8 {excess[8]:.3f}, "
          f"j=19 {excess[19]:.3f}  (пол D(1) = {c[1]:.3f})")
