...
def main() -> None:
    ...
    print("=" * 74)
    print("  Числа фазы A (0.40 / 0.79 / 0.91) получены на ДРУГОЙ выборке и на\n"
          "  ЕДИНОМ офсете 3, поэтому не переносятся. Прежнее объяснение через\n"
          "  длину паддинга ОПРОВЕРГНУТО зондом k4b0_padding_probe: при левом\n"
          "  дополнении разрыв в position ids нулевой при любой длине.\n"
    ...
    # СВЕРКА «ровно K» против «<= K» НА ВСЕХ СТРОКАХ. Равенство гарантировано
    # лишь когда набор можно добить позициями ВНЕ support: нужно
    # 16 - |C| >= K - |S*|. При крупном support запаса может не быть, и «ровно
    # K» вынужден брать вредные позиции внутри support.
    #
    # Условие проверяется ВЕКТОРНО и потому на всех 16000 строках, а не на
    # подвыборке: лучший набор лежит внутри support по построению сжатой
    # таблицы, поэтому достаточно popcount(support) и best_size_by_k. Прежде
    # здесь стоял rng.choice(n, size=min(3000, n)), и вывод «расхождений 0»
    # распространялся на весь датасет без основания.
    supp_sz = np.array([bin(int(x)).count("1") for x in lb["support"]])
    free_all = P - supp_sz
    slack = free_all - (kmax - lb["best_size_by_k"][:, kmax].astype(np.int64))
    n_bad = int((slack < 0).sum())
    print(f"\n  «ровно K» против «<=K» на ВСЕХ {n} строках: нарушений {n_bad}, "
          f"запас 16-|C|-(K-|S*|) мин {int(slack.min())}, "
          f"среднее {slack.mean():.2f}")
    print("    распределение запаса: " + " ".join(
        f"{v}:{c / n:.1%}" for v, c in
        zip(*np.unique(np.clip(slack, -1, 8), return_counts=True))))
    assert n_bad == 0, "набор <=K нельзя добить нейтральными позициями до ровно K"

    # НЕЗАВИСИМАЯ перепроверка того же вывода прямым перебором таблицы: она
    # медленная, поэтому идёт по подвыборке и служит контролем рассуждения выше,
    # а не источником вывода.
    ...
    print(f"    контроль перебором на {min(3000, n)} строках: расходятся в "
          f"{n_diff} случаях, макс. разница {worst_d:.3e}")
    print(f"    changed-support: среднее {supp_sz.mean():.2f} позиции из {P}, "
          f"свободных вне support {free_all.mean():.2f}")
