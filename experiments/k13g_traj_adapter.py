...
def bound_norm(dz, torch):
    ...
    сравнивать величину, меньшую до sqrt(n_pos) раз, с тем же порогом: проверка
...
def selftest():
    ...
    # Позиционная формула даёт величину не более чем в sqrt(n_pos) раз и
    # прошла бы предел всегда. Равенство достигается, когда поправка
    # сосредоточена в одной позиции. Проверяется, что числа разные и что
