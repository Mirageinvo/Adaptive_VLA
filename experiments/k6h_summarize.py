...
def selftest():
    ...
    # МЯГКИЙ РЕЖИМ ПРОВЕРЯЕТСЯ С ПЕРЕХВАТОМ ВЫВОДА. Он печатает
    # «ВНИМАНИЕ: ячейки от РАЗНЫХ прогонов», а самопроверка гоняется при
    # КАЖДОМ запуске анализа — в логе настоящего прогона эта строка читалась
    # бы как предупреждение о смеси, которой нет.
    import contextlib as _cl
    import io as _io
    _buf = _io.StringIO()
    with _cl.redirect_stdout(_buf):
        assert check_single_run_tag(["k11e", "k11e_b"], allow=True) is False
    assert "РАЗНЫХ прогонов" in _buf.getvalue(), "мягкий режим должен сообщать"
