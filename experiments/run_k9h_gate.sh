...
# ДВОЕТОЧИЯ ЗДЕСЬ НЕТ НАМЕРЕННО. `${X:-по умолчанию}` подставляет умолчание и
# когда переменная НЕ ЗАДАНА, и когда она ПУСТАЯ, поэтому `EXPECT_SOURCE=`
# молча возвращало бы frozen12_rstar. `${X-по умолчанию}` различает эти
# случаи, и пустое значение остаётся пустым.
