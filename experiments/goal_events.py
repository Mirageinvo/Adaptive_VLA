...
def bucketize(remaining, edges):
    """Индекс бакета. side="left", иначе подписи не соответствуют содержимому.

    При side="right" значение, равное границе, уходило в СЛЕДУЮЩИЙ бакет:
    остаток 8 попадал в «8-16», хотя нулевой бакет подписан «<= 8». Ошибка
    была на каждой границе и смещала веса ближнего бакета, по которому и
    выносится вердикт.
    """
...
def selftest():
    ...
    # ЗНАЧЕНИЕ РОВНО НА ГРАНИЦЕ обязано лежать в бакете со своей подписью.
    ed = [8, 16, 32, 48]
    nm = bucket_names(ed)
    for r, want in ((0, "<= 8"), (7, "<= 8"), (8, "<= 8"), (9, "8-16"),
                    (16, "8-16"), (17, "16-32"), (48, "32-48"), (49, "> 48")):
        got = nm[int(bucketize([r], ed)[0])]
