...
def main() -> None:
    ...
    for Bs in [int(x) for x in args.batches.split(",")]:
        ...
        # r = 1 - (16 + 7K) / (8*16) от стоимости refiner.
        #
        # ОБОЗНАЧЕНИЯ, ИХ ЛЕГКО ПЕРЕПУТАТЬ. r — СЭКОНОМЛЕННАЯ ДОЛЯ стоимости
        # refiner, а НЕ его ускорение; ускорение refiner равно S_ref = 1/(1-r).
        # f — доля refiner в сквозном времени, R — сквозное ускорение.
        # Закон Амдала: R = 1/(1 - f*r), отсюда f >= (1 - 1/R)/r. Запись через
        # ускорение refiner, f >= (1 - 1/R)/(1 - 1/S_ref), — ТА ЖЕ формула.
        # ВНИМАНИЕ: «ускорение 1.15x» и «снижение времени на 15%» — РАЗНЫЕ
        # требования (1.15x это -13.0%); берём ускорение.
        ...
        print(f"{'K':>6}{'экономия refiner r':>20}{'= S_ref':>10}"
              f"{'для 1.15x':>12}{'для 1.25x':>12}")
        for K in (1, 2, 4, 8):
            r = 1.0 - (16 + 7 * K) / (8 * 16)
            print(f"{K:>6}{r:>19.1%}{1 / (1 - r):>9.2f}x"
