...
# Сетка штрафа. НУЛЬ ВКЛЮЧЁН ЯВНО и стоит первым: при 1.5 млн строк на 1281
# признак штраф может быть не нужен вовсе, и это законный ответ, а не
# ненайденный оптимум. Прежняя сетка начиналась с 1e-6, оптимум упирался в
# неё, и правило справедливо объявляло вердикт недействительным.
LAMBDAS = (0.0,) + tuple(float(x) for x in np.logspace(-12.0, 2.0, 29))
N_BOOT = 1000
BOOT_SEED = 21
INNER_SEED = 13
INNER_FRAC = 0.2
...
# --------------------------------------------------------------------------
def make_solver(G, C, idx, bias_i, rcond=1e-10):
    """Решатель через СОБСТВЕННОЕ РАЗЛОЖЕНИЕ, один раз на набор признаков.

    ПОЧЕМУ НЕ `solve`. Нормальные уравнения возводят число обусловленности в
    квадрат, признаков 1281 и они коррелированы, а оптимальный штраф оказался
    порядка 1e-8 — то есть практически нулевым. `np.linalg.solve` на такой
    матрице исключения НЕ бросает: он возвращает неустойчивые коэффициенты
    молча. Ловить это через try/except бесполезно — ветка не срабатывает
    никогда. Предыдущая попытка чинить только случай `lam == 0` мимо цели:
    выбранная λ нулём не была, и путь через SVD не включался.

    Разложение симметричной матрицы НЕ зависит от штрафа, поэтому делается
    один раз, а каждое значение λ — это сдвиг собственных чисел. Заодно
    отсекаются направления, где `d + lam` ниже допуска: они и есть источник
    неустойчивости.
    """
    idx = list(idx)
    ...
    Ms[:, dead] = 0.0
    Ms = 0.5 * (Ms + Ms.T)
    d, V = np.linalg.eigh(Ms)
    d = np.clip(d, 0.0, None)          # мелкие отрицательные — от округления
    ybar = C[bias_i] / n
    ...
    def solve(lam):
        dl = d + float(lam)
        cut = max(dmax * rcond, 0.0)
        keep = dl > cut
        w_s = V[:, keep] @ (Ct[keep] / dl[keep, None])
        w = w_s / std[:, None]
        w[dead, :] = 0.0
        b = ybar - mean @ w
        dk = dl[keep]
        info = dict(n_features=len(idx), eff_rank=int(keep.sum()),
                    cond=float(dk.max() / dk.min()) if dk.size else
                    float("inf"),
                    eig_max=dmax,
                    eig_min=float(d.min()) if d.size else 0.0,
                    # СЫРОЕ число обусловленности, БЕЗ отсечения: именно
                    # оно показывает, насколько задача была бы неустойчива,
                    # если решать её напрямую. Прежняя версия считала его
                    # уже после отсечения и потому всегда выглядела хорошо.
                    cond_raw=(float(dmax / d.min())
                              if d.size and d.min() > 0 else float("inf")),
...
def ridge_from_gram(G, C, idx, lam, bias_i, return_info=False):
    """Обёртка над `make_solver` для одиночного решения."""
    w, b, info = make_solver(G, C, idx, bias_i)(lam)
...
def selftest():
    ...
    # --- нуль в сетке вычислим и стоит первым ------------------------------
    assert LAMBDAS[0] == 0.0, "нуль обязан быть в сетке и первым"
    assert LAMBDAS[-1] > 1.0, "верхний край сетки должен быть большим"
    rng0 = np.random.default_rng(9)
    n0, d0, k0 = 300, 5, 2
    X0 = rng0.normal(size=(n0, d0))
    X0[:, 3] = X0[:, 0]          # намеренно вырожденный признак
    Xa0 = np.hstack([X0, np.ones((n0, 1))])
    Y0 = X0[:, :1] * 2.0 + rng0.normal(size=(n0, k0)) * 0.01
    ...
    assert i0["eff_rank"] < d0, ("разложение не отсекло вырожденное "
                                 f"направление: ранг {i0['eff_rank']} из {d0}")
    assert i0["dropped"] >= 1 and i0["cond_raw"] > 1e12, i0
    # ОБУСЛОВЛЕННОСТЬ ОТЧИТЫВАЕТСЯ ПРИ ЛЮБОМ ШТРАФЕ, а не только при нуле:
    # выбранная на реальных данных λ была порядка 1e-8, и прежняя версия,
    # чинившая только случай ровного нуля, до этого пути не доходила.
    i1 = ridge_from_gram(Gn, Cn, list(range(d0)), 1.0, d0,
                         return_info=True)[2]
    assert i1 and i1["eff_rank"] == d0, i1
    assert i1["cond"] < i0["cond_raw"] / 100, (i1["cond"],
                                               i0["cond_raw"])
    # МАЛЫЙ, НО НЕНУЛЕВОЙ штраф обязан оставаться плохо обусловленным —
    # именно этот случай и встретился на данных
    i2 = ridge_from_gram(Gn, Cn, list(range(d0)), 1e-8, d0,
                         return_info=True)[2]
    assert i2["cond"] > i1["cond"] * 100, (i2["cond"], i1["cond"])
    # Совпадение с прямым решением на ХОРОШО обусловленной задаче проверяют
    # тесты восстановления известной связи ниже — они идут через тот же
    # решатель, поэтому отдельного сравнения здесь не нужно.
    ...
    print("самопроверка k11p пройдена (версия «решение через собственное "
          "разложение»): "
          ...
          "отвергают подмену нормы, отвода, meta и массивов, решение идёт "
          "через собственное разложение и отсекает неустойчивые направления, "
          "обусловленность отчитывается при ЛЮБОМ штрафе и остаётся плохой "
          "при малом ненулевом, а собранный конвейер находит заложенную "
          "связь и НЕ находит её там, где её нет")
...
def main() -> None:
    ...
    best, edge_vars = {}, []
    # РАЗЛОЖЕНИЕ ОДИН РАЗ НА ВАРИАНТ. Оно не зависит от штрафа, поэтому все
    # тридцать значений — это сдвиг собственных чисел, а не тридцать решений.
    solv_it = {v: make_solver(A["it"]["G"], A["it"]["Cclip"], IDX[v], bias_i)
    ...
    for v in ("z0", "h24", "both"):
        ...
        k_ = int(np.argmin(sc))
        best[v] = LAMBDAS[k_]
        # ГРАНИЦЫ НЕ РАВНОЦЕННЫ. Упереться в ВЕРХНИЙ край значит, что сетка
        # мала и оптимум не найден. Нижний край здесь — точный нуль, то есть
        # естественная граница области определения: за ней ничего нет, и
        # «штраф не нужен» — законный ответ. Недействительным вердикт делает
        # только верхний край.
        at_edge = k_ == len(LAMBDAS) - 1
        edge_vars += [v] if at_edge else []
        edge = (" (на ВЕРХНЕМ краю сетки — вердикт недействителен)"
                if at_edge else
                " (штраф не нужен: оптимум в нуле)" if k_ == 0 else "")
        print(f"    {v:>5}: lam = {best[v]:.3g}, MSE = {sc[k_]:.6f}{edge}")

    # --- ПОДГОНКА НА ВСЁМ TRAIN --------------------------------------------
    W, cond_info = {}, {}
    for v in ("z0", "h24", "both"):
        wc, bc, ic = make_solver(full["G"], full["Cclip"], IDX[v],
                                 bias_i)(best[v])
        wr, br, _ = make_solver(full["G"], full["Craw"], IDX[v],
                                bias_i)(best[v])
        W[v] = dict(clip=(wc, bc), raw=(wr, br))
        cond_info[v] = ic
    # ОБУСЛОВЛЕННОСТЬ ПЕЧАТАЕТСЯ ВСЕГДА, а не только при нулевом штрафе.
    # Выбранная λ порядка 1e-8 нулём не является, но и не регуляризует:
    # без этих чисел нельзя сказать, осмысленны ли коэффициенты.
    print(f"\n  обусловленность (решение через собственное разложение):")
    print(f"    {'вход':>6}{'lam':>10}{'эфф.ранг':>11}{'из':>7}"
          f"{'cond при lam':>15}{'cond сырое':>13}{'отброшено':>11}")
    for v in ("z0", "h24", "both"):
        ic = cond_info[v]
        print(f"    {v:>6}{best[v]:>10.2g}{ic['eff_rank']:>11}"
              f"{ic['n_features']:>7}{ic['cond']:>15.2e}"
              f"{ic['cond_raw']:>13.2e}{ic['dropped']:>11}")
    print(f"    (отброшены направления, где собственное число со штрафом ниже "
          f"1e-10 от\n     наибольшего — именно они и дают неустойчивость)")
    ...
    if edge_vars:
        ...
        verdict = (
            ...
            f"пришлась на ВЕРХНИЙ край сетки ({LAMBDAS[-1]:.1g}), то есть "
            f"оптимум не найден и штрафа не хватило. Расширьте сетку вверх и "
            f"повторите; числа ниже — разведочные.\n  " + verdict)
    print(f"\n  {verdict}")
