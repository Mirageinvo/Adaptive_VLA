...
for S in s0 s1; do
    ...
    LOG="logs/k13d/${S}.log"
    # ПРОПУСКА ГОТОВОГО НЕТ, И ЭТО НАМЕРЕННО. Калибровка стоит 26 секунд, а
    # пропуск по наличию непустого файла принял бы артефакт, снятый другой
    # версией скрипта, с другим горизонтом или другой головой. Чтобы пропускать
    # безопасно, пришлось бы сверять здесь весь провенанс — дороже, чем
    # пересчитать.
    if [ -s "$OUT" ]; then
        echo "[$(ts)] $S: $OUT перезаписывается (пересчёт дешевле сверки)"
    fi
    echo "[$(ts)] $S: старт"
