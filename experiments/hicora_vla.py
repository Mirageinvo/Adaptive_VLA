...
def make_residual_head():
    ...
    class ResidualHead(nn.Module):
        ...
        def forward(self, h, z0):
            ...
            НИ ОДНОГО ПЕРЕХОДА НА CPU: ни проверок через приведение тензора
            к числу, ни подсчёта насыщения. Насыщение считается снаружи по
            возвращённым коэффициентам и только в диагностическом режиме.
            Готовность базиса — обычный булев атрибут на стороне CPU, его
...
def selftest(numpy_only=False):
    ...
    # БЕЗ БАЗИСА forward ОТКАЗЫВАЕТ. Проверка идёт по булеву атрибуту CPU,
    # поэтому синхронизации не создаёт, а забытый set_basis иначе дал бы
    # молча нулевую поправку.
    try:
        head(h, z0)
