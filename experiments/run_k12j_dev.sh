...
# --- ПАРАМЕТРЫ АЛГОРИТМА: ОТДЕЛЬНЫМ ФАЙЛОМ, А НЕ В config.json -----------
# В config.json их вносить нельзя: строка сверяется на равенство, и добавление
# поля сделало бы уже посчитанные каталоги «чужими». Здесь то же требование —
# зафиксировать и сверять, — но для файла, которого у старых каталогов просто
# нет: отсутствующий создаётся, существующий сверяется. Иначе повторный запуск
# того же каталога с другим окружением продолжил бы ДРУГОЙ алгоритм под видом
# того же прогона.
RP="$ROOT/run_params.json"
RPNEW=$(printf '{"lr":"%s","halvings":%s,"ladder":"%s","target":%s,"max_steps":%s,"kl_max":%s,"ratio_max":%s,"ratio_hard":%s}' \
  "$LR" "$HALVINGS" "$LADDER" "$TARGET" "$MAXSTEPS" \
  "${KL_MAX:-0.02}" "${RATIO_MAX:-1.5}" "${RATIO_HARD:-3.0}")
if [ -f "$RP" ]; then
  if [ "$(cat "$RP")" != "$RPNEW" ]; then
    echo "параметры алгоритма в $ROOT не совпадают с запрошенными:"
    echo "  было:  $(cat "$RP")"
    echo "  стало: $RPNEW"
    echo "продолжение этого каталога другим алгоритмом не допускается"; exit 1
  fi
else
  printf '%s' "$RPNEW" > "$RP"
fi
...
while [ "$DONE" -lt "$TARGET" ]; do
  N=$DONE; K=$((DONE + 1))
  # §40.1: ПЕРВЫЙ no_step ТЕРМИНАЛЕН. Повтор того же шага с новым буфером —
  # это повторение до успеха, и число обновлений перестало бы быть
  # зарегистрированной величиной. Проверка здесь дублирует ту, что внутри
  # k12e: там она ловит прямой запуск, здесь — перезапуск каталога.
  if [ -s "$ROOT/step_$K.json" ] \
     && grep -q '"status": *"no_step"' "$ROOT/step_$K.json"; then
    say "ШАГ $K УЖЕ ЗАКОНЧИЛСЯ no_step — по §40.1 это терминально. Каталог "\
"$ROOT не перезапускается, ветка не проходит гейт"
    exit 3
  fi
