...
def persistence(events, n_steps, horizon):
    ...
    # ТА ЖЕ СЕМАНТИКА ГРАНИЦЫ, ЧТО В goal_events.targets: side="left",
    # действие переключения принадлежит завершающейся фазе. Прежде здесь
    # стояло "right", и K-10c считал по другой границе, чем K-10d и K-10e —
    # при том что унификация была заявлена.
    goal_id = np.searchsorted(ev, calls, side="left")
