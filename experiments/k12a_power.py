...
def r_loss_max(p_fail, delta):
    """Предельная допустимая доля потерь ПРИ ПОЛНОМ восстановлении провалов.

    Мой прежний вывод «если r_loss не ниже 5%, эффект +5 пп невозможен
    независимо от восстановлений» арифметически неверен. Предел зависит от
    опоры:

        r_loss_max = (p_fail - delta) / (1 - p_fail),

    и при успешности 89.5% он равен 6.15%, при 92.0% — 3.26%, при 90.25% —
    5.26%. При восстановлении ниже 100% допустимая потеря ещё меньше.

    И отдельно: `r_loss` нельзя использовать как единственный показатель.
    НЕИЗМЕНИВШАЯСЯ политика имеет r_loss = 0 и не улучшает ничего. Нужны
    одновременно восстановление, потеря и чистый эффект.
    """
    p_succ = 1.0 - p_fail
    if p_succ <= 0:
        return None
    v = (p_fail - delta) / p_succ
...
def selftest():
    ...
    # --- предел потерь зависит от ОПОРЫ -------------------------------------
    for pf, want in ((0.105, 0.0615), (0.080, 0.0326), (0.0975, 0.0526)):
        got = r_loss_max(pf, 0.05)
        assert abs(got - want) < 1e-3, (pf, got, want)
    # эффект выше потолка -> предела нет
    assert r_loss_max(0.08, 0.09) is None
    # при нулевом эффекте терять можно ровно столько, сколько есть провалов
    assert abs(r_loss_max(0.105, 0.0) - 0.105 / 0.895) < 1e-12
