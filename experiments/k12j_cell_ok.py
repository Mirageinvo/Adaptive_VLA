...
def compare(meta, want):
    """Расхождения между ожидаемым и записанным. Отсутствие поля — расхождение:
    старая ячейка без provenance не годится для пропуска."""
    cfg = dict(meta.get("run_config") or {})
    bad = []
    for k, v in sorted(want.items()):
        got = cfg.get(k, meta.get(k, None))
        if got is None:
            bad.append(f"{k}: в ячейке нет поля, ожидалось {v}")
            continue
        try:
...
def selftest():
    ...
    assert compare(m, dict(sigma=0.03))[0].startswith("sigma:")
    assert compare(m, dict(step_index=3))[0].startswith("step_index:")
    assert compare(m, dict(eval_eps_seed=777))[0].startswith("eval_eps_seed:")
    ...
    # ячейка без provenance для пропуска не годится
    r = os.path.join(d, "old.json")
    json.dump(dict(episodes=[]), open(r, "w"))
    assert compare(read_meta(r), dict(sigma=0.1))
...
def main():
    ...
    for a in sys.argv[2:]:
        ...
        k, v = a.split("=", 1)
        want[k] = v
    bad = compare(meta, want)
    if bad:
        print(f"{sys.argv[1]}: " + "; ".join(bad), file=sys.stderr)
        return 2
    return 0
