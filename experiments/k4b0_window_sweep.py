...
def main() -> None:
    ...
    print("\n  ВОРОТА ПО УСЛОВНЫМ BASELINE, записаны до запуска.\n"
          ...
          "  ОРАКУЛЬНАЯ версия S(p) — верхняя граница любого метода, знающего\n"
          "  только p. Ворота применять к НЕЙ: обученный S(p) занижен шумом\n"
          "  отбора лучшей из 1820 масок, и разрыв от него оптимистичен.")
    # Правило про длину окна печатается только когда окон действительно
    # несколько: в скалярном режиме оно сбивает с толку.
    if not SCALAR:
        print("\n  ЧИТАТЬ ТАК, правило записано до запуска.\n"
              "  Если на длинных окнах фикс-маска сходится со случайным\n"
              "  выбором — объяснение через геометрию декодера полное. Если\n"
              "  точный оракул при этом держится заметно выше matched-random,\n"
              "  а Жаккар лучших наборов низкий — состояние-зависимое\n"
              "  множество существует. K/16 = 0.25 — лишь интуитивный\n"
              "  ориентир для аддитивно-равномерного случая, нулевым baseline\n"
              "  считается ИЗМЕРЕННЫЙ случайный выбор.\n"
              "  ПРОТОКОЛЬНЫМ остаётся окно 4: исполняются четыре шага.")
