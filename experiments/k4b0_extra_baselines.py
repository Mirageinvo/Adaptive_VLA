...
def static_analysis(tables, lb, e0, iv, it, epi, tsk, P, K, tr, res, ft_off):
    ...
    2. ЛУЧШАЯ ФИКСИРОВАННАЯ МАСКА, подобранная по фактическому G на validation
       ...
       baseline. Перебираются ВСЕ C(16,4) = 1820 наборов: ограничение десятью
       лучшими позициями профиля было предвзято в пользу самого профиля —
       набор, сильный за счёт взаимодействий слабых по одиночке позиций, в
       кандидаты не попадал.
