...
def read_leak(same_len_diff, batch_effect, storage=5e-4, near=3.0):
    ...
    return "shape", (
        f"ЗАГРЯЗНЕНИЯ НЕТ: при одинаковой длине после набивки смена "
...
def selftest():
    ...
    tag_, txt_ = read_leak(1.5e-2, 1.9e-2)
    assert tag_ == "leak" and "ЛОЖНЫЙ" in txt_, txt_
    tag_, txt_ = read_leak(2e-4, 1.9e-2)
    assert tag_ == "shape" and "ЗАГРЯЗНЕНИЯ НЕТ" in txt_, txt_
    # КОНТРОЛЬ: «загрязнения нет» не выдаётся при расхождении в проценты
    for v_ in (1e-2, 5e-3, 2e-3):
        assert read_leak(v_, 1.9e-2)[0] == "leak", v_
    ...
    print("самопроверка k11q пройдена (версия «интервенция и проверка загрязнения»): "
          ...
          "загрязнение содержимым соседей — от шума формы батча")
...
def main() -> None:
    ...
    leak_diff, leak_note = None, "не выполнено"
    ...
    if len(pool_le) >= 2:
        ...
        if pair is None:
            ...
            if sorted(set(ln_c)) != sorted(set(ln_d)):
                ...
                print(f"\n  загрязнение: соседи одной длины "
                      f"{sorted(set(ln_c))}, содержимое разное — целевая "
                      f"строка меняется на {leak_diff:.2e}")
    else:
    ...
    out = dict(script_sha1=k11a.file_sha1(__file__), cache=args.cache,
               ...
               leak=dict(diff=leak_diff, note=leak_note, tag=leak_tag),
               intervention=dict(
