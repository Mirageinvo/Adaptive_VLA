...
# измерение идёт час, а его числа идут в FINDINGS. Поэтому set -e, ff-only
# pull, самопроверка перед запуском и отказ при существующих выходных файлах.
#
#   bash experiments/k14c_run_eval.sh              # cuda:1, сид 0, метка v2
#   bash experiments/k14c_run_eval.sh cuda:0 0 v3
set -euo pipefail
DEV="${1:-cuda:1}"
SEED="${2:-0}"
TAG="${3:-v2}"
